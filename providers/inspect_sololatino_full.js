/**
 * Auditoría Integral y Diagnóstico Profundo para SoloLatino / Embed69
 * Ejecución: node providers/inspect_sololatino_full.js 615 tv 1 1
 *             node providers/inspect_sololatino_full.js 550 movie
 */

var tmdbId = process.argv[2] || "615";
var mediaType = process.argv[3] || "tv";
...
console.log("================================================================");
console.log("[*] AUDITORÍA EXHAUSTIVA DE SOLOLATINO / EMBED69");
console.log("    ID TMDB: " + tmdbId + " | Tipo: " + mediaType + (isTv ? " (T" + seasonNum + " E" + episodeNum + ")" : ""));
console.log("================================================================\n");

// ==========================================
// CRIPTO NATIVA HERMES SAFE
// ==========================================
...
console.log("[1] TMDB: Obteniendo metadatos...");
var tmdbUrl = "https://api.themoviedb.org/3/" + (isTv ? "tv" : "movie") + "/" + tmdbId + "?api_key=" + TMDB_API_KEY + "&language=es-MX&append_to_response=external_ids";

fetch(tmdbUrl)
    .then(function(r) { return r.json(); })
    .then(function(meta) {
        var imdbId = (meta.external_ids && meta.external_ids.imdb_id) || meta.imdb_id;
        console.log("-> Título:", meta.name || meta.title);
        console.log("-> IMDb ID:", imdbId);

        if (!imdbId) throw new Error("Sin IMDb ID");

        var targetUrl = isTv ? (BASE_URL + "/f/" + imdbId + "-" + seasonNum + "x" + ("0" + episodeNum).slice(-2)) : (BASE_URL + "/f/" + imdbId);
        console.log("\n[2] Embed69: Conectando a:", targetUrl);
        var t0 = Date.now();

        return fetch(targetUrl, { headers: { "User-Agent": USER_AGENT, "Referer": BASE_URL + "/" } })
            .then(function(res) {
                console.log("-> Status HTTP:", res.status, "(" + (Date.now() - t0) + " ms)");
                return res.text();
            })
            .then(function(html) {
                ...
                if (!challengeMatch || !dataLinkMatch) throw new Error("No se detectó PoW en HTML");
                ...
                console.log("\n[3] Minando PoW (Dificultad " + difficulty + ")...");
                var tPoW = Date.now();
                var nonce = solvePoW(challenge, difficulty);
                console.log("-> Nonce:", nonce, "en " + (Date.now() - tPoW) + " ms");
                ...
                for (var i = 0; i < dataLink.length; i++) {
                    var f = dataLink[i];
                    if (f.sortedEmbeds) {
                        for (var j = 0; j < f.sortedEmbeds.length; j++) {
                            var emb = f.sortedEmbeds[j];
                            var dec = decryptAES(emb.link, aesKey);
                            if (dec) links.push({ server: emb.servername, lang: f.video_language, url: dec });
                        }
                    }
                }

                console.log("\n[4] Enlaces descifrados (" + links.length + "):");
                links.forEach(function(l, idx) {
                    console.log("   [" + (idx + 1) + "] " + l.server + " (" + l.lang + ") -> " + l.url);
                });

                console.log("\n[5] PRUEBAS DE EXTRACCIÓN Y VALIDACIÓN EN VIVO:");

                var tests = links.map(function(item) {
                    var u = item.url;
                    var sName = (item.server || "").toLowerCase();

                    // --- PRUEBA STREAMWISH ---
                    if (sName.indexOf("streamwish") !== -1 || u.indexOf("hglink") !== -1 || u.indexOf("streamwish") !== -1) {
                        var id = u.split("?")[0].replace(/\/$/, "").split("/").pop();
                        var wishDirectUrl = "https://streamwish.to/e/" + id;
                        console.log("\n[*] Testeando StreamWish vía:", wishDirectUrl);
                        var tw = Date.now();

                        return fetch(wishDirectUrl, { headers: { "User-Agent": USER_AGENT, "Referer": "https://streamwish.to/" } })
                            .then(function(r) { return r.text(); })
                            .then(function(txt) {
                                var unpacked = unpackJS(txt);
                                var m3u8 = (unpacked ? unpacked.match(/https?:\/\/[^"'\s<>\\]+\.m3u8[^"'\s<>]*/i) : null) || txt.match(/https?:\/\/[^"'\s<>]+\.m3u8[^"'\s<>]*/i);
                                if (m3u8) {
                                    console.log("   ✓ StreamWish extrajo m3u8 en " + (Date.now() - tw) + " ms");
                                    console.log("     URL m3u8:", m3u8[0].substring(0, 80) + "...");
                                    // Validar stream en vivo
                                    return fetch(m3u8[0], { headers: { "User-Agent": USER_AGENT, "Referer": "https://streamwish.to/" } })
                                        .then(function(vr) {
                                            console.log("     -> Validación EN VIVO StreamWish: HTTP " + vr.status + " (" + vr.headers.get("content-type") + ")");
                                        });
                                } else {
                                    console.log("   ✗ StreamWish: No se pudo extraer m3u8");
                                }
                            })
                            .catch(function(e) { console.log("   ✗ StreamWish Error:", e.message); });
                    }

                    // --- PRUEBA VIDHIDE ---
                    if (sName.indexOf("vidhide") !== -1 || u.indexOf("morencius") !== -1 || u.indexOf("vidhide") !== -1) {
                        console.log("\n[*] Testeando VidHide vía:", u);
                        var tv = Date.now();

                        return fetch(u, { headers: { "User-Agent": USER_AGENT, "Referer": BASE_URL + "/" } })
                            .then(function(r) { return r.text(); })
                            .then(function(txt) {
                                var unpacked = unpackJS(txt);
                                var m3u8 = (unpacked ? unpacked.match(/https?:\/\/[^"'\s<>\\]+\.m3u8[^"'\s<>]*/i) : null) || txt.match(/https?:\/\/[^"'\s<>]+\.m3u8[^"'\s<>]*/i);
                                if (m3u8) {
                                    ...
                                    console.log("   ✓ VidHide extrajo m3u8 en " + (Date.now() - tv) + " ms");
                                    console.log("     URL m3u8:", streamUrl.substring(0, 80) + "...");

                                    // Probar validación en vivo con Referer raíz
                                    var refRaiz = "https://morencius.com/";
                                    return fetch(streamUrl, { headers: { "User-Agent": USER_AGENT, "Referer": refRaiz } })
                                        .then(function(vr) {
                                            console.log("     -> Validación EN VIVO VidHide (con " + refRaiz + "): HTTP " + vr.status + " (" + vr.headers.get("content-type") + ")");
                                        });
                                } else {
                                    console.log("   ✗ VidHide: No se pudo extraer m3u8");
                                }
                            })
                            .catch(function(e) { console.log("   ✗ VidHide Error:", e.message); });
                    }
